...
bool font_6x8_getbit(char character, uint8_t x, uint8_t y);

static const uint8_t font_6x8[] ICACHE_RODATA_ATTR = {
...
// Get pixel for character at (x, y): 0 <= x < 6, 0 <= y < 8
bool font_6x8_getbit(char character, uint8_t x, uint8_t y) {
	if (x >= 6 || y >= 8)
		return false;

	uint32_t res;
	const uint8_t *addr = &font_6x8[character * 8 + y];
